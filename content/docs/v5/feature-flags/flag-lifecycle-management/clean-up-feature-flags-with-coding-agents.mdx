...
- Use [Set cleanup expectations](./set-cleanup-expectations.mdx) to define when different flag types should be reviewed or removed.
- Use [Detect stale feature flags](./detect-stale-feature-flags.mdx) to identify candidate flags from FeatBit state, tags, audit history, rollout state, experiment state, and repository usage.
- Use the convention from [Implement feature flags](./implement-feature-flags.mdx) so each flag has predictable keys, registry entries, evaluation helpers, and consumption points that are easy to find later.
