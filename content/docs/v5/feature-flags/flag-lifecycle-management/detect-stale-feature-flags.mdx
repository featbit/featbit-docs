...
After you have already [set cleanup expectations](/v5/feature-flags/flag-lifecycle-management/set-cleanup-expectations), stale flag detection should be handled by a reusable coding-agent skill. The agent only needs to read your cleanup rules, query FeatBit through [FeatBit CLI](/v5/feature-flags/flag-lifecycle-management/create-flags-with-agent-guidance#use-featbit-cli) or [FeatBit MCP](/v5/feature-flags/flag-lifecycle-management/create-flags-with-agent-guidance#use-featbit-mcp), search the repository for flag usage, and return a short cleanup review.
...
The full pattern is already demonstrated in the [stale flag review example](/v5/feature-flags/flag-lifecycle-management/set-cleanup-expectations#stale-flag-review-example), where a coding agent uses project rules, repository references, and FeatBit state to list stale feature flags.
