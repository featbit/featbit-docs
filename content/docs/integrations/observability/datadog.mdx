...
1. On the menu, under **Integrations**, select **Webhooks** to access the webhook configuration page.
2. Click the **+ New webhook** button.
3. In the 'New webhook' drawer, enter the **Name** for the webhook.
4. Enter the URL of the DataDog Events API. You can find the API URL in the [DataDog official tutorial](https://docs.datadoghq.com/api/latest/events/?code-lang=curl#post-an-event).
5. Select the scopes of WebHook you wish to subscribe to by clicking the **Choose environments** button.
6. Select the **Events** you want to subscribe to. For this tutorial, select **Flag Change** and **Segment**.
...
**For more information about WebHook, please refer this [tutorial](/docs/integrations/webhooks).**
...
1. You need to first create a Flag Trigger in a Feature Flags. See [Workflow/Flag Triggers](/docs/feature-flags/feature-workflow/flag-triggers).
