...
FeatBit supports two different connectivity modes and two different deployment modes

### Connectivity modes

FeatBit Agent supports two different connectivity modes:

* Online: Connection to FeatBit Server, supports dynamic feature flags, segments and environment keys synchronization and other advanced features.
...
FeatBit Agent supports two different deployment modes:

- Standalone: each agent has its own data store and not sharing with other agents.
- Cluster: multiple agents share the same data store (Redis).

Only one leader agent exists at a given time. When no leader agent exists, one of the slave agents would be elected as the new leader.
How to maintain the status of data store when running in cluster mode:
Initially, cluster mode only supports Redis as data store. In redis, we keep a dict of connected agents:
Agents: {agent_1: {latest_active_timestamp,is_leader}, agent_2: {latest_active_timestamp,is_leader}}
When an agent starts, it adds itself to redis if not exist already. It then checks if an active leader exists:
- if it founds an active leader, then it is read-only to redis (it still updates its status to redis).
- If no active leader exists, it becomes the leader and is reponsible of maintainning the data in store
Every agent should report its status to redis every X seconds, it should verify if an active leader exists, if not, it registers itself as the new leader.

There is no Leader Agent when running in non offline mode