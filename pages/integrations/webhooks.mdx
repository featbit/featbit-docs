...
3. Compare the signature that FeatBit sent (the value of the `X-Hub-Signature-256` header) with the signature that you computed, and ensure that they match.
...
<Callout type="info">
    If we're doing a redelivery, the `X-GitHub-Delivery` header will be the same as in the original delivery.
</Callout>