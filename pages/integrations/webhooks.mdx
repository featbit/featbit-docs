...
2. In your code that handles webhook deliveries, you should calculate a hash using your `secret` token and the `payload` contents.
3. Compare the hash that FeatBit sent (the value of the `X-Hub-Signature-256` header) with the hash that you calculated, and ensure that they match.
